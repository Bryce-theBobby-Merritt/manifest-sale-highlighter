...
**Tasks**  
- [ ] Use a reliable Excel parsing library (for example, `xlsx` in Node.js).  
- [ ] Parse row by row, capturing "Article No." and "US Sale" columns.  
- [ ] Store the result in an internal data structure or temporary DB table.
...
**Tasks**  
- [ ] For each row, check if "US Sale" is empty or not.  
- [ ] Mark row internally as "sale_item = true" or "sale_item = false".
...
**Tasks**  
- [ ] Use a PDF parsing library or approach that can extract text.  
- [ ] Implement regex or text-based logic to identify valid Article Nos (format to be confirmed).  
- [ ] Store extracted item numbers in a data structure or DB table.
...
### 2.4 US-F2.4: Match Sale Items to Manifest
...
**Tasks**  
- [ ] Implement intersection logic between "sale_items" and "pdf_items".  
- [ ] Handle edge cases (duplicates, partial matches if required).  
- [ ] Return or store matched items in a final data structure.
...
**Priority**: Medium  
**Tasks**  
- [ ] Implement front-end UI for progress feedback.  
- [ ] Optionally poll the server for completion status.  
- [ ] Display "Processing" if not done.
